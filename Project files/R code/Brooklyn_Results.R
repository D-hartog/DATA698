...
model_report <- read_csv("https://raw.githubusercontent.com/D-hartog/DATA698/refs/heads/main/Project%20files/Data/Results%20data/brooklyn_model_report.csv")
brooklyn_fc_accuracy <- read_csv("https://raw.githubusercontent.com/D-hartog/DATA698/refs/heads/main/Project%20files/Data/Results%20data/brooklyn_fc_accuracy.csv")

head(brooklyn_fc_accuracy)


best_rmse <- brooklyn_fc_accuracy %>% group_by(nypd_pct_cd, call_type) %>% slice_min(RMSE, n = 1)
...
acf1 <- brooklyn_fc_accuracy %>% select(nypd_pct_cd, call_type, .model, ACF1)
...
combined_diagnostics %>% select(nypd_pct_cd, call_type, .model, white_noise) %>% 
  ...
  theme(plot.title = element_text(size = 20, face = "bold", hjust = 0.5),
        ...
        panel.grid.minor = element_blank(), # Removes minor grid lines
        strip.text = element_text(colour = "black")) +
  labs(x = "Model name", y = "Precinct", title = "Brooklyn: CIP Models with White Noise Residuals")
...
combined_diagnostics %>% select(nypd_pct_cd, call_type, .model, white_noise) %>% 
  ...
  theme(plot.title = element_text(size = 20, face = "bold", hjust = 0.5),
        ...
        panel.grid.minor = element_blank(), # Removes minor grid lines
        strip.text = element_text(colour = "black")) +
  labs(x = "Model name", y = "Precinct", title = "Brooklyn: Non CIP Models with White Noise Residuals")

# _________XGBoost_________

xg_recipe<- read_csv("https://raw.githubusercontent.com/D-hartog/DATA698/refs/heads/main/Project%20files/Data/Results%20data/xgb_recipe_summary_bk.csv")
brooklyn_fc_accuracy_xg <- read_csv("https://raw.githubusercontent.com/D-hartog/DATA698/refs/heads/main/Project%20files/Data/Results%20data/BK_XGBOOST_accuracy.csv")
xg_best_rmse <- read_csv("https://raw.githubusercontent.com/D-hartog/DATA698/refs/heads/main/Project%20files/Data/Results%20data/BK_XGBOOST_best_rmse.csv")
...
comparisons %>% filter(call_type != "<aggregated>") %>% 
  ...
  theme(plot.title = element_text(size = 20, face = "bold", hjust = 0.5),
        ...
        panel.grid.minor = element_blank(), # Removes minor grid lines
        strip.text = element_text(colour = "black")) +
  labs(title = "Brooklyn: XGBOOST vs. ARIMA/ETS", x = "Model", y = "Number of models")
