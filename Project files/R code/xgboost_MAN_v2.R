...
daily_counts_type <- read_csv("https://raw.githubusercontent.com/D-hartog/DATA698/refs/heads/main/Project%20files/Data/manhattan_dailyCIP_counts.csv")
holidays <- read_csv("https://github.com/D-hartog/DATA698/blob/main/Project%20files/Data/Holidays_cleaned.csv")
...
daily_counts_type_ts %>% filter(nypd_pct_cd %in% precint_sample) %>% 
  ...
  theme(plot.title = element_text(size = 20, face = "bold", hjust = 0.5),
        ...
        panel.grid.minor = element_blank(), # Removes minor grid lines
        strip.text = element_text(colour = "black")) + 
  labs(title = "Manhattan: Sample Precinct Time Series",
       x = "Date", y = "Number of calls")


# Zoomed in Data from sample Precincts - October 2024
daily_counts_type_ts %>% filter(nypd_pct_cd %in% c(1,10,19,24), 
...
daily_counts_type_ts$boro_id <- "Manhattan"
...
write_csv(recipe_summary, "xgb_recipe_summary_man.csv")
...
write_csv(test_forecast_table, "XGBOOST_forecast_table_man.csv")
...
## Boro
filter_identifier_boro <- "Manhattan"
...
test_forecast_table %>% filter(identifier %in% c("1", "10", "19", "24")) %>% 
...
test_forecast_table %>% filter(identifier == 1) %>% 
  group_by(identifier, call_type) %>% 
  filter_by_time(
    .date_var = .index,
    .start_date = "2025-06-13",
...
write_csv(accuracy_by_identifier, "MAN_XGBOOST_accuracy.csv")

best_rmse <- accuracy_by_identifier %>% group_by(identifier, call_type) %>% 
  slice_min(rmse, n = 1) %>% ungroup()

write_csv(best_rmse, "MAN_XGBOOST_best_rmse.csv")
...
write_csv(poisson_parameters, "manhattan_fit_params.csv")

# Results visualization
xg_best_rmse %>% ggplot(aes(y = fct_rev(fct_infreq(name)))) + geom_bar(fill = "black") + 
  ...
  theme(plot.title = element_text(size = 20, face = "bold", hjust = 0.5),
        ...
        panel.grid.minor = element_blank(), # Removes minor grid lines
        strip.text = element_text(colour = "black")) +
  labs(title = "Model with Lowest RMSE: Manhattan", x = "Model", y = "Number of models")
